...
面向 AI Agent 与协作者的项目说明。在创建新页面、修改现有页面或编写组件时，**必须严格遵循本项目的设计语言、Theme 配置和原子组件规范**。
...
- **遵循设计语言与 Theme**：在修改、添加任何页面、组件或样式时，必须严格应用 `app/globals.css` 中的 `:root` 变量（间距、字体、字号、字重、圆角、阴影等），保持高对比度深色磨砂纯黑的极客风格。
- **复用原子 UI 组件**：优先并强制复用 `components/ui/` 中的原子组件（`Button`、`Card`、`Terminal`、`Table`）。如果要创建的新组件属于基础通用原子层，也应抽象至该目录下，避免在业务代码中直接手写繁琐的自定义按钮或卡片标签。
- 保持动改范围小，只动与任务相关的文件
